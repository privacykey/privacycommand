...
public enum KnowledgeBase {
    ...
    private static let _allArticles: [KnowledgeArticle] = [
        ...
        .init(
            ...
            summary: "A plain-English reading of what a binary can do, built from its import table and linked libraries — not from a full disassembly.",
            detail: "What matters to most people isn't the raw `mov`/`add`/`bl` instructions — it's **what the binary asks the operating system to do**. That answer lives at two boundaries that are trivially readable straight out of the Mach-O file, without disassembling a single instruction:\n\n  • **Imported symbols** — the undefined external symbols a binary references are the libc / framework functions it calls (`_dlopen`, `_SecItemCopyMatching`, `_connect`, `_CGEventTapCreate`). Auditor reads the symbol table directly and categorises them by purpose (file I/O, networking, keychain, shell, …).\n  • **Linked libraries** — the `LC_LOAD_DYLIB` list reveals the high-level capability surface: linking `Security.framework` means it can touch the keychain; `AVFoundation` means camera/microphone; `CoreLocation` means location, and so on. For modern framework-based apps this is the *dominant* signal — such an app often imports zero raw socket calls yet clearly does networking because it links `CFNetwork`.\n\nThese facts are readable even on **encrypted Mac App Store binaries** (the `__LINKEDIT` symbol table isn't encrypted) and on stripped binaries — exactly the cases where a text disassembler returns nothing. When a local `objdump`/`otool` is installed and succeeds, Auditor layers instruction-level detail (instruction counts, per-function network call sites, and a Ghidra Decompile affordance) on top as optional enrichment.\n\nThis is **evidence of capability, not proof of behaviour**. A referenced API means the code *can* do something — not that it does at runtime. Use the live monitored run to confirm."
        ),
        .init(
